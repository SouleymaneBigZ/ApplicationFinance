...
# Mode de calcul
mode = st.selectbox("Choisissez le mode de calcul", ["Mode Classique", "Mode Optimisé"])
...
if mode == "Mode Classique":
    ...
    # Paramètres du prêt
    taux_interet_annuel = st.number_input("Taux d'intérêt annuel (en fraction)", value=0.02)
    mensualite_souhaitee = st.number_input("Mensualité souhaitée par ménage (en MAD)", value=1000.0)
    
    # Choix de l'optimisation : Durée ou Montant
    optimisation = st.radio("Optimiser pour :", ["Durée du prêt", "Montant du prêt"])
    ...
    # Apport initial
    apport_initial = 0.1 * contribution_menages
    duree_remboursement = 1

    if optimisation == "Durée du prêt":
        montant_pret_total = 0.9 * contribution_menages
        montant_pret_par_menage = montant_pret_total / nombre_menages

        # Calcul du nombre de paiements nécessaires pour atteindre la mensualité souhaitée
        nombre_paiements = int(np.log(1 - (montant_pret_par_menage * (taux_interet_annuel / 12) / mensualite_souhaitee)) / np.log(1 + taux_interet_annuel / 12))
        duree_remboursement = nombre_paiements // 12

    elif optimisation == "Montant du prêt":
        nombre_paiements = duree_remboursement * 12
        montant_pret_par_menage = mensualite_souhaitee * (1 - (1 + taux_interet_annuel / 12) ** -nombre_paiements) / (taux_interet_annuel / 12)
        montant_pret_total = montant_pret_par_menage * nombre_menages
...
duree_remboursement_annees = nombre_paiements // 12
annees = np.arange(1, duree_remboursement_annees + 1)
solde_initial_annee = np.zeros(duree_remboursement_annees)
paiement_annuel_total = np.zeros(duree_remboursement_annees)
principal_annuel_total = np.zeros(duree_remboursement_annees)
interet_annuel_total = np.zeros(duree_remboursement_annees)
solde_final_annee = np.zeros(duree_remboursement_annees)

for i in range(duree_remboursement_annees):
